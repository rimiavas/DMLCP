...
function preload() {
  // load image
  img = loadImage("images/obama.jpg"); // try alf.jpg, worf.jpg
  // IDEA: make handling images easier for the user? For instance:
  //         - let the user try the different images by flipping between them
  //         - implement an image upload mechanism
  //         - use an external api to pull an image from the internet
}
...
function drawLandmarks(detections){
  ...
  */
  ...
  // mouth
  beginShape();
  detections.parts.mouth.forEach(item => vertex(item._x, item._y))
  endShape(CLOSE);

  // nose
  beginShape();
  detections.parts.nose.forEach(item => vertex(item._x, item._y))
  endShape(CLOSE);

  // left eye
  beginShape();
  detections.parts.leftEye.forEach(item => vertex(item._x, item._y))
  endShape(CLOSE);

  // right eye
  beginShape();
  detections.parts.rightEye.forEach(item => vertex(item._x, item._y))
  endShape(CLOSE);

  // right eyebrow
  beginShape();
  detections.parts.rightEyeBrow.forEach(item => vertex(item._x, item._y))
  endShape();

  // left eye
  beginShape();
  detections.parts.leftEyeBrow.forEach(item => vertex(item._x, item._y))
  endShape();
